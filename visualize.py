...
def draw_border(img, top_left, bottom_right, vh_type, color=(0, 255, 0), thickness=10, line_length_x=200,
                line_length_y=200):
    x1, y1 = top_left
    x2, y2 = bottom_right
...
while ret:
    ...
    if ret:
        ...
        for row_indx in range(len(df_)):
            # draw car
            car_x1, car_y1, car_x2, car_y2, vh_type = ast.literal_eval(
                df_.iloc[row_indx]['car_bbox'].replace('[ ', '[').replace('   ', ' ').replace('  ', ' ').replace(' ',
                                                                                                                 ',').replace(
                    ' ', ','))

            draw_border(frame, (int(car_x1), int(car_y1)), (int(car_x2), int(car_y2)), vh_type, (0, 255, 0), 25,
